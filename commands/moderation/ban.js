...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('ban')
        .setDescription('Execute a formal ban of a member')
    ...
    async execute(interaction, client) {
        ...
        if (!member.bannable) return interaction.reply({ content: '❌ Security Protocol: Target user cannot be banned by this bot.', ephemeral: true });

        await member.ban({ reason });

        const embed = new EmbedBuilder()
            .setColor('#D32F2F')
            .setAuthor({ name: 'Moderation Action: Ban', iconURL: user.displayAvatarURL() })
            .addFields(
        ...
        await logger.log(client, interaction.guild, {
            isMod: true,
            title: '🛡️ Moderation Log: Member Ban',
            color: '#D32F2F',
